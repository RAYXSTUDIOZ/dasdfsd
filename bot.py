...
async def on_ready():
    ...
    # --- AutoMod rule creation (will give Uses AutoMod badge) ---
    try:
        guild = bot.get_guild(int(os.getenv("AUTOMOD_GUILD_ID", "1311717154256851057")))
        if guild:
            # Try creating a simple keyword block rule (Discord will ignore duplicates)
            await guild.create_automod_rule(
                name="AutoMod Badge Rule",
                event_type=1,  # MESSAGE_SEND
                trigger_type=1,  # KEYWORD
                trigger_metadata={"keyword_filter": ["automodtestword"]},
                actions=[{"type": 1}]  # BLOCK_MESSAGE
            )
            logger.info("AutoMod rule created (or already exists). Badge should appear soon.")
        else:
