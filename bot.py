...
# -------------------------
# INJECTED: Per-guild config storage & management (injected by ChatGPT)
# -------------------------
import json as __json_injected
from pathlib import Path as __Path_injected

__BASE_DIR_injected = __Path_injected(__file__).parent
__GUILD_CONFIGS_FILE_injected = __BASE_DIR_injected / "guild_configs.json"
__GUILD_INVITERS_FILE_injected = __BASE_DIR_injected / "guild_inviters.json"

def __load_json_injected(path, default):
    try:
        if path.exists():
            with open(path, "r", encoding="utf-8") as f:
                return __json_injected.load(f)
    except Exception as e:
        print("Injected: error loading", path, e)
    return default

def __save_json_injected(path, data):
    try:
        with open(path, "w", encoding="utf-8") as f:
            __json_injected.dump(data, f, indent=2, ensure_ascii=False)
    except Exception as e:
        print("Injected: error saving", path, e)

# load or create templates if missing
if not __GUILD_CONFIGS_FILE_injected.exists():
    default_template = {
        "123456789012345678": {
            "antispam": True,
            "anti_raid": True,
    ...
    __save_json_injected(__GUILD_CONFIGS_FILE_injected, default_template)

if not __GUILD_INVITERS_FILE_injected.exists():
    __save_json_injected(__GUILD_INVITERS_FILE_injected, {})

# helper functions used by injected commands
def get_guild_config_injected(guild_id: int):
    gid = str(guild_id)
    cfgs = __load_json_injected(__GUILD_CONFIGS_FILE_injected, {})
    if gid not in cfgs:
        cfgs[gid] = {
            "antispam": True,
            "anti_raid": True,
        ...
        __save_json_injected(__GUILD_CONFIGS_FILE_injected, cfgs)
    return cfgs[gid]

def set_guild_config_value_injected(guild_id: int, key: str, value):
    cfgs = __load_json_injected(__GUILD_CONFIGS_FILE_injected, {})
    gid = str(guild_id)
    if gid not in cfgs:
        cfgs[gid] = {}
...
def load_guild_inviters_injected():
    return __load_json_injected(__GUILD_INVITERS_FILE_injected, {})

def save_guild_inviters_injected(data):
    __save_json_injected(__GUILD_INVITERS_FILE_injected, data)

# -------------------------
# INJECTED: Interactive setup, settings, panel and announce commands (guarded)
# -------------------------
try:
    from discord import ui as __ui_injected, ButtonStyle as __ButtonStyle_injected, Embed as __Embed_injected
    from discord.ext import commands as __commands_injected
except Exception as e:
    print("Injected: discord import failed", e)
else:
    # Setup view
    class __SetupViewInjected(__ui_injected.View):
        def __init__(self, guild, author, timeout=180):
            super().__init__(timeout=timeout)
            self.guild = guild
            self.author = author

        async def interaction_check(self, interaction):
            # allow original inviter or guild admin or the invoker
            try:
                if interaction.user.id == self.author.id:
                    return True
                from inspect import getmodule
                # fallback: allow guild owner / administrators
        ...
        @__ui_injected.button(label="Toggle Anti-Spam", style=__ButtonStyle_injected.primary)
        async def toggle_antispam(self, button, interaction):
            cfg = get_guild_config_injected(self.guild.id)
            new = not cfg.get("antispam", True)
            set_guild_config_value_injected(self.guild.id, "antispam", new)
            await interaction.response.edit_message(content=f"Anti-Spam set to **{new}**", view=self)

        @__ui_injected.button(label="Toggle Anti-Raid", style=__ButtonStyle_injected.primary)
        async def toggle_antiraid(self, button, interaction):
            cfg = get_guild_config_injected(self.guild.id)
            new = not cfg.get("anti_raid", True)
            set_guild_config_value_injected(self.guild.id, "anti_raid", new)
            await interaction.response.edit_message(content=f"Anti-Raid set to **{new}**", view=self)

        @__ui_injected.button(label="Toggle Chat Mode", style=__ButtonStyle_injected.secondary)
        async def toggle_chatmode(self, button, interaction):
            cfg = get_guild_config_injected(self.guild.id)
            new = not cfg.get("chat_mode", False)
            set_guild_config_value_injected(self.guild.id, "chat_mode", new)
            await interaction.response.edit_message(content=f"Chat Mode set to **{new}**", view=self)

        @__ui_injected.button(label="Set Log Channel", style=__ButtonStyle_injected.success)
        async def set_logchannel(self, button, interaction):
            await interaction.response.send_message("Please mention the channel to use as log channel here within 60s or type 'none'.", ephemeral=True)
            def check(m):
                return m.author.id == interaction.user.id and m.channel == interaction.channel
            try:
                msg = await __bot_wait_for_injected('message', check=check, timeout=60)
                if msg.content.lower() == 'none':
                    set_guild_config_value_injected(self.guild.id, "log_channel_id", None)
                    await interaction.followup.send("Log channel cleared.", ephemeral=True)
                else:
                    chan = msg.channel_mentions[0] if msg.channel_mentions else None
                    if chan:
                        set_guild_config_value_injected(self.guild.id, "log_channel_id", chan.id)
                        await interaction.followup.send(f"Log channel set to {chan.mention}", ephemeral=True)
                    else:
                        await interaction.followup.send("Couldn't find that channel. Try again.", ephemeral=True)
            except Exception as e:
                await interaction.followup.send("No response. Log channel not changed.", ephemeral=True)

    # Panel view
    class __PanelViewInjected(__ui_injected.View):
        def __init__(self, guild, author, timeout=180):
            super().__init__(timeout=timeout)
            self.guild = guild
            self.author = author
        ...
        async def interaction_check(self, interaction):
            if interaction.user.id == self.author.id or (hasattr(interaction.user,'guild_permissions') and interaction.user.guild_permissions.administrator):
                return True
            await interaction.response.send_message("You are not allowed to use this.", ephemeral=True)
            return False
        ...
        async def toggle_antispam(self, button, interaction):
            ...
            new = not cfg.get("antispam", True)
            set_guild_config_value_injected(self.guild.id, "antispam", new)
            await interaction.response.send_message(f"Anti-Spam set to **{new}**", ephemeral=True)
        ...
        async def toggle_chatmode(self, button, interaction):
            ...
            new = not cfg.get("chat_mode", False)
            set_guild_config_value_injected(self.guild.id, "chat_mode", new)
            await interaction.response.send_message(f"Chat Mode set to **{new}**", ephemeral=True)

        @__ui_injected.button(label="Set Min Age", style=__ButtonStyle_injected.success)
        async def set_min_age(self, button, interaction):
            await interaction.response.send_message("Reply with min account age in days (e.g., 7) or 'none' to disable. You have 60s.", ephemeral=True)
            def check(m):
                return m.author.id == interaction.user.id and m.channel == interaction.channel
            try:
                msg = await __bot_wait_for_injected('message', check=check, timeout=60)
                ...
                if txt == 'none':
                    set_guild_config_value_injected(self.guild.id, "min_account_age_days", 0)
                    await interaction.followup.send("Minimum account age disabled.", ephemeral=True)
                else:
                    try:
                        days = int(txt)
                        set_guild_config_value_injected(self.guild.id, "min_account_age_days", days)
                        await interaction.followup.send(f"Minimum account age set to {days} days.", ephemeral=True)
                    except:
                        await interaction.followup.send("Couldn't parse that as a number. Try again.", ephemeral=True)
            except Exception:
                await interaction.followup.send("No response. Value not changed.", ephemeral=True)

    # The commands will only be added if not already defined in the original file
    try:
        # helper to use bot.wait_for without circular import
        __bot_wait_for_injected = globals().get('bot').wait_for if globals().get('bot') else None

        if __bot_wait_for_injected is None:
            # if bot isn't in globals, try to import it by name (fallback)
            try:
                from __main__ import bot as __main_bot_injected
                __bot_wait_for_injected = __main_bot_injected.wait_for
            except:
                __bot_wait_for_injected = None

        # Register commands only if names not used
        def __register_command_if_absent(name, func):
            existing = any(re.search(fr"@bot\.command\(name=['\"]{re.escape(name)}['\"]\)", orig_text) for _ in [0])
            if existing:
                return False
            try:
                globals()['bot'].command(name=name)(func)
                return True
            except Exception as e:
                try:
                    # fallback decorator
                    __main_bot_injected.command(name=name)(func)
                    return True
                except Exception as ee:
                    print("Injected: failed to register command", name, e, ee)
                    return False

        async def __setup_cmd_injected(ctx):
            # Permission check
            author = ctx.author
            guild = ctx.guild
            if not (author.guild_permissions.manage_guild or author.guild_permissions.administrator):
                await ctx.send("You need to be a server admin or the inviter to use the setup.")
                return
            cfg = get_guild_config_injected(guild.id)
            embed = __Embed_injected(title="Server Setup Wizard", description=(
                f"Configure core settings for **{guild.name}**.\\n\\n"
                f"Anti-Spam: **{cfg.get('antispam', True)}**\\n"
                f"Anti-Raid: **{cfg.get('anti_raid', True)}**\\n"
                f"Chat Mode: **{cfg.get('chat_mode', False)}**\\n"
                f"Min Account Age: **{cfg.get('min_account_age_days', 7)}** days\\n"
                f"Log Channel: **{('<#%s>'%cfg['log_channel_id']) if cfg.get('log_channel_id') else 'Not set'}**\\n\\n"
                "Use the buttons below to toggle or set values."
            ), color=0x00AAFF)
            view = __SetupViewInjected(guild, author)
            await ctx.send(embed=embed, view=view)

        async def __settings_cmd_injected(ctx, key: str = None, value: str = None):
            author = ctx.author
            guild = ctx.guild
            if not (author.guild_permissions.manage_guild or author.guild_permissions.administrator):
                await ctx.send("You need to be a server admin or the inviter to use this command.")
                return
            cfg = get_guild_config_injected(guild.id)
            if not key:
                pretty = "\\n".join([f"• {k}: {v}" for k, v in cfg.items()])
                await ctx.send(f"**Current settings for {guild.name}:**\\n{pretty}")
                return
            key = key.lower()
            if key in ('antispam','anti_raid','chat_mode'):
                if not value:
                    await ctx.send("Please provide a value: on/off")
                    return
                val = value.lower()
                if val in ('on','true','yes','1'):
                    set_guild_config_value_injected(guild.id, key, True)
                elif val in ('off','false','no','0'):
                    set_guild_config_value_injected(guild.id, key, False)
                else:
                    await ctx.send("Invalid value. Use 'on' or 'off'.")
                    return
                await ctx.send(f"{key} set to {get_guild_config_injected(guild.id)[key]}")
                return
            if key in ('min_account_age_days','min_age','minage'):
                try:
                    if value and value.lower() == 'none':
                        set_guild_config_value_injected(guild.id, 'min_account_age_days', 0)
                    else:
                        days = int(value)
                        set_guild_config_value_injected(guild.id, 'min_account_age_days', days)
                    await ctx.send(f"Minimum account age set to {get_guild_config_injected(guild.id)['min_account_age_days']} days")
                except:
                    await ctx.send("Invalid number. Example: !settings min_age 7")
                return
            if key in ('log_channel','log_channel_id'):
                if ctx.message.channel_mentions:
                    chan = ctx.message.channel_mentions[0]
                    set_guild_config_value_injected(guild.id, 'log_channel_id', chan.id)
                    await ctx.send(f"Log channel set to {chan.mention}")
                    return
                else:
                    try:
                        cid = int(value)
                        set_guild_config_value_injected(guild.id, 'log_channel_id', cid)
                        await ctx.send(f"Log channel set to <#{cid}>")
                        return
                    except:
                        await ctx.send("Please mention a channel or provide a channel ID.")
                        return
            await ctx.send("Unknown setting key.")

        async def __panel_cmd_injected(ctx):
            author = ctx.author
            guild = ctx.guild
            if not (author.guild_permissions.manage_guild or author.guild_permissions.administrator):
                await ctx.send("You need to be a server admin or the inviter to open the panel.")
                return
            cfg = get_guild_config_injected(guild.id)
            embed = __Embed_injected(title="Server Control Panel", description=(
                f"Anti-Spam: **{cfg.get('antispam')}**\\n"
                f"Anti-Raid: **{cfg.get('anti_raid')}**\\n"
                f"Chat Mode: **{cfg.get('chat_mode')}**\\n"
                f"Min Account Age: **{cfg.get('min_account_age_days')}** days\\n"
                f"Log Channel: **{('<#%s>'%cfg['log_channel_id']) if cfg.get('log_channel_id') else 'Not set'}**\\n"
            ), color=0x44ff44)
            view = __PanelViewInjected(guild, author)
            await ctx.send(embed=embed, view=view)

        async def __announce_cmd_injected(ctx, *, message: str):
            # Only bot owner allowed (safety)
            try:
                if not await globals().get('bot').is_owner(ctx.author):
                    await ctx.send("Only the bot owner can use this command.")
                    return
            except Exception:
                # Fallback simple check (owner id env) - best effort
                owner_id_env = os.getenv("BOT_OWNER_ID")
                if owner_id_env and str(ctx.author.id) != str(owner_id_env):
                    await ctx.send("Only the bot owner can use this command.")
                    return
            sent = 0
            failed = 0
            inviters = load_guild_inviters_injected()
            for gid_str, inviter_id in inviters.items():
                try:
                    gid = int(gid_str)
                    gcfg = get_guild_config_injected(gid)
                    if not gcfg.get('announce_enabled', True):
                        continue
                    guild = globals().get('bot').get_guild(gid)
                    owner = None
                    if guild:
                        owner = guild.get_member(inviter_id) or guild.owner
                    if owner:
                        try:
                            await owner.send(f"📢 Announcement from the bot owner:\\n\\n{message}")
                            sent += 1
                        except:
                            failed += 1
                except:
                    failed += 1
            await ctx.send(f"Announcement attempt finished. Sent: {sent}, Failed: {failed}")

        # Attempt to register commands if they do not exist already in original file
        try:
            # Use the bot in globals
            __botobj = globals().get('bot')
            if __botobj:
                # Only register if not present in original code
                if 'setup' not in [c.name for c in __botobj.commands]:
                    __botobj.command(name='setup')(__setup_cmd_injected)
                if 'settings' not in [c.name for c in __botobj.commands]:
                    __botobj.command(name='settings')(__settings_cmd_injected)
                if 'panel' not in [c.name for c in __botobj.commands]:
                    __botobj.command(name='panel')(__panel_cmd_injected)
                if 'announce' not in [c.name for c in __botobj.commands]:
                    __botobj.command(name='announce')(__announce_cmd_injected)
            else:
                # If bot not in globals at import time, register later by attaching functions to module-level names
                globals()['__setup_cmd_injected'] = __setup_cmd_injected
                globals()['__settings_cmd_injected'] = __settings_cmd_injected
                globals()['__panel_cmd_injected'] = __panel_cmd_injected
                globals()['__announce_cmd_injected'] = __announce_cmd_injected
        except Exception as e:
            print("Injected: error registering commands", e)

    except Exception as e:
        print("Injected: error during view/command injection", e)
